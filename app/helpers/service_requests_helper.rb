module ServiceRequestsHelper
  ...
  def user_actions(request)
    if %w[Completed Cancelled Rejected].include?(request.status.try(:name))
      (link_to t('.show', default: t('helpers.links.show')), service_request_path(request), class: 'btn btn-xs btn-primary')
    elsif %w[Pending InCompleted].include?(request.status.try(:name))
  ...
  def service_request_header_html_block
    columns = [t('.service'), t('.address'), t('.user'), t('.service_due_date'),
               t('.time_slot'), t('.service_request_number'), t('.status'),
               t('.rating'), t('.actions')]
