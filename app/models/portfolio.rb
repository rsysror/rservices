class Portfolio < ApplicationRecord
  ...
  validates :about, :experience, :company_ph_no,
            :address, :company_name, presence: true, on: :update
  ...
  def available_time_slots(date)
    time_slots = service_requests.present? ? TimeSlot.ordered - service_requests.where(service_date: date).map(&:time_slot).compact : TimeSlot.ordered
  ...
  def available_time_slots_for_update(service_request)
    time_slots = service_requests.present? ? TimeSlot.ordered - service_requests.where(service_date: service_request.service_date).map(&:time_slot).compact : TimeSlot.ordered
    all_slots = (service_request.service_date.strftime('%d/%m/%Y') == Time.now.strftime('%d/%m/%Y')) ? select_time_slots(time_slots) : time_slots
    all_slots << service_request.time_slot
  end
  ...
  def get_all_services(page, per_page)
    portfolio_services.order(:id).paginate(page: page, per_page: per_page || 5)
  end
  ...
  def remove_existing_service_city(service_id)
    portfolio_services.where(service_id: service_id).map(&:city)
  end  
end
