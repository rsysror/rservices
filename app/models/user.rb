...
class User < ApplicationRecord
  ...
  has_many :assigned_service_requests, class_name: 'ServiceRequest',
  ...
  def get_user_service_requests(page)
    service_requests.includes(:service, :address, :status, :portfolio,
                              :time_slot)
                    .ordered.paginate(page: page, per_page: 5)
  end

  def get_partner_service_requests(page)
    portfolio.service_requests.includes(:service, :address, :status, :portfolio,
                                        :time_slot)
             .ordered.paginate(page: page, per_page: 5)
  end
